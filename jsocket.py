...
class JsonSocket:
        ...
        def __init__(self, sock=None,udp=None):
                if sock is None:
                        if udp == None or udp == False:
        ...
        def send_obj(self, obj, encode_func=json.dumps):
                ...
                if self._socket:
                        ...
                        format_str = "={0}s".format(len(msg))
                        msg_packed = struct.pack(format_str, msg.encode('utf-8'))
                        msg_length = len(msg_packed)
                        header_packed = struct.pack('!H', msg_length)
...
class JsonServer(JsonSocket):
        ...
        def __init__(self, address=None, port=None, udp=False):
                # Python 3
                #super().__init__()
...
class JsonClient(JsonSocket):
        def __init__(self, address=None, port=None,_udp=None):
                # Python 3
                #super().__init__()

                super(JsonClient, self).__init__(None,_udp)
